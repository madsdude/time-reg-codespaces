...
// helpers
function parseTimeHHMM(s){ if(!/^\d{2}:\d{2}$/.test(s||'')) return null; const [h,m]=s.split(':').map(Number); if(h<0||h>23||m<0||m>59) return null; return {h,m}; }
function minutesBetween(st,et){ let d=(et.h*60+et.m)-(st.h*60+st.m); if(d<0) d+=24*60; return d; }

// fælles query (uden pause i output)
function buildEntriesQuery(from,to){
  let sql=`
    SELECT te.id, te.work_date, te.start_time, te.end_time,
...
// --- API ---
app.get('/api/users', async(_req,res)=>{ try{
  const r=await pool.query('SELECT id,name FROM users ORDER BY name;'); res.json(r.rows);
}catch(e){ console.error(e); res.status(500).json({error:'DB fejl'}); } });

app.get('/api/time-entries', async(req,res)=>{ try{
  const {from,to}=req.query; const {sql,params}=buildEntriesQuery(from,to);
  const r=await pool.query(sql,params); res.json(r.rows);
}catch(e){ console.error(e); res.status(500).json({error:'DB fejl'}); } });

app.post('/api/time-entries', async(req,res)=>{ try{
  ...
  const st=parseTimeHHMM(start_time), et=parseTimeHHMM(end_time);
  ...
  const project_id=pr.rows[0]?.id; if(!project_id) return res.status(500).json({error:'Intet projekt i DB (kræves teknisk)'});
  const dur=span; // ingen pause
  const r=await pool.query(
    ...
    [user_id, project_id, work_date, start_time, end_time, 0, dur, note||null]
  );
  res.status(201).json(r.rows[0]);
}catch(e){ console.error(e); res.status(500).json({error:'DB fejl'}); } });

// opsummering pr. person (json)
app.get('/api/summary', async(req,res)=>{ try{
  ...
  const out=r.rows.map(x=>({ user_id:x.user_id, user_name:x.user_name, entries:Number(x.entries),
    minutes:Number(x.minutes||0), hours:Number(((x.minutes||0)/60).toFixed(2)) }));
  res.json(out);
}catch(e){ console.error(e); res.status(500).json({error:'DB fejl'}); } });

// CSV (detalje)
app.get('/api/export.csv', async(req,res)=>{ try{
  const {from,to}=req.query; const {sql,params}=buildEntriesQuery(from,to);
  const r=await pool.query(sql,params);
  ...
  res.send('\ufeff'+lines.join('\n'));
}catch(e){ console.error(e); res.status(500).json({error:'Eksport CSV fejlede'}); } });

// Excel (detalje)
app.get('/api/export.xlsx', async(req,res)=>{ try{
  const {from,to}=req.query; const {sql,params}=buildEntriesQuery(from,to);
  const r=await pool.query(sql,params);
  const wb=new ExcelJS.Workbook(); const ws=wb.addWorksheet('Tid (detaljer)');
  ws.columns=[
    {header:'Dato',key:'dato',width:12},{header:'Person',key:'person',width:22},
    {header:'Start',key:'start',width:10},{header:'Slut',key:'slut',width:10},
    {header:'Minutter',key:'minutter',width:10},{header:'Timer',key:'timer',width:10},
...
}catch(e){ console.error(e); res.status(500).json({error:'Eksport Excel fejlede'}); } });

// *** NYT: Excel (opsummering) ***
app.get('/api/export-summary.xlsx', async(req,res)=>{ try{
  const {from,to}=req.query;
  let sql=`
  ...
  const r=await pool.query(sql,params);
  const wb=new ExcelJS.Workbook(); const ws=wb.addWorksheet('Opsummering');
  ws.columns=[
  ...
  for(const row of r.rows){
    ...
    ws.addRow({person:row.user_name, entries:Number(row.entries||0), minutes:mins, hours:hours});
  }
  // total række
  ...
  await wb.xlsx.write(res); res.end();
}catch(e){ console.error(e); res.status(500).json({error:'Eksport summary Excel fejlede'}); } });

app.listen(PORT, async()=>{ try{
  await initDb();
  console.log(`Tidregistrering v2.2 kører på http://0.0.0.0:${PORT}`);
}catch(e){ console.error('DB init fejl:',e); process.exit(1); }});
